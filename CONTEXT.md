...
1. **$99 1-2-1 session — Cal.com setup COMPLETE (2026-07-09), ready to push.** Event type "Solar Return Walk Through" created, Stripe app connected, real booking URL `https://cal.com/staleys/solar-return-walk-through` (note: slug is `solar-return-walk-through`, dash before "through" — not the originally planned `solar-return-walkthrough`). Placeholder href in `index.html` swapped to this real URL (~line 255). Confirmed set: 15-min buffer after, 24hr-notice/~4-week booking window, evening/weekend-only availability, all 6 custom booking-form fields. **Test booking completed successfully** — incognito test booking (Saturday 11 July, 12:00pm, 60 min, $99) reached Cal.com's Stripe payment page requiring card details before confirmation, proving the payment gate works; test was cancelled before paying. Confirmation message field couldn't be edited (likely a Cal.com paid-event-type quirk — different tab/plan restriction, not yet root-caused, low priority). **Next step: push to GitHub** (see git push workflow below) — nothing else blocking go-live.
...
- ✓ **1-on-1 section sub-copy rewritten** (2026-07-09) — Dropped "sometimes you just want a witch in the room with you" line (Staley didn't like it). New copy: "The course hands you the skill. This is the hour we put it into practice together — live, just the two of us, walking through your solar return chart side by side. We'll turn every placement into something you can actually *use* this year." Now echoes the Cal.com event description's opening line ("The course hands you the skill...") instead of duplicating language across the two surfaces.
- ✓ **"Read it with me" 1-on-1 section redesigned — night sky spotlight** (2026-07-09) — Staley felt the session section blended into the course section above it (same purple family, same button style). Presented 3 design options (night sky spotlight / invitation ticket / split layout with chart orb); chose Option A. `css/main.css`: `.session-section` background changed from the mid-purple gradient (matching the course section) to a near-black radial (`#1A0645` → `#0E0530` → `#05001A`) for sharp contrast. `.session-heading` gets a soft magenta text-shadow glow. `.session-card` background lightened to a faint white wash with a magenta border + inset/outset glow, plus `✦` corner accents via `::before`/`::after`. New `.session-cta-btn` class (solid magenta `#E040A8` fill, hover to pink `#F880C0`) added alongside the shared `.course-cta-btn` base on the booking button in `index.html`, so the 1-on-1 CTA no longer matches the course's white pill button.
- ✓ **Cal.com booking link live in `index.html`** (2026-07-09) — Event type "Solar Return Walkthrough" created on Cal.com, Stripe app connected, real URL `https://cal.com/staleys/solar-return-walk-through` swapped into the `Book Your Session — $99 →` button (placeholder comment removed). Buffer/availability/notice-window/custom-fields/confirmation-message settings still need to be confirmed as set — see "What's Next."
- ✓ **$99 1-on-1 session section built** (2026-06-28) — New "1-on-1" section added to `index.html`, placed directly after the course section as a $27→$99 upsell. Eyebrow "1-on-1 · $99 · One Hour", heading "Read it with me.", in-voice copy ("sometimes you just want a witch in the room with you"), a "What the hour holds" card (4 items: rising sign/role, Sun & Moon, houses asking for attention, your questions answered), and a `Book Your Session — $99 →` CTA. New `.session-*` styles in `css/main.css` (gradient bridges course #5A1488 → about #5A1488); reuses `.block-eyebrow`, `.out-glyph`, `.waitlist-wrap`, `.course-cta-btn`. "1-on-1" links added to nav (`#session`) and footer. **Booking link is a placeholder** (`https://cal.com/YOUR-USERNAME/solar-return-walkthrough`) marked with an HTML comment above the button — swap once the Cal.com event is live. Decisions: 60 min, Cal.com (free-plan Stripe payments), solar return walkthrough framing.
